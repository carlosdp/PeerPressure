...
async function job(job: Job<ProcessPhotosJob>) {
  logger.info(`Processing photos for profile ${job.data.profileId}`);
  ...
  const unprocessedPhotos = photoKeys.filter((photo) => !photo.description);

  if (unprocessedPhotos.length === 0) {
    logger.debug("No unprocessed photos");
    return;
  }

  const downloadedPhotos = await Promise.all(
    unprocessedPhotos.map(async (photoKey) => {
...
async function analyzePhotos(
  ...
  const photosWithDescriptions = photos.map((
    ...
    description: photo.description ??
      photoDescriptions.find((d: { key: string }) => d.key === photo.key)
        ?.description,
  }));
